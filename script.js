...
let musicStarted = false; // Nueva variable para controlar la música
...
// Nueva función para iniciar la música con el primer clic del usuario
function startMusicWithUserClick() {
    if (!musicStarted) {
        menuMusic.play();
        musicStarted = true;
    }
    window.removeEventListener('click', startMusicWithUserClick);
}

// Espera el primer clic en la ventana para iniciar la música del menú
window.addEventListener('click', startMusicWithUserClick);
// Ajustar el tamaño del canvas a la ventana
function resizeCanvas() {
    canvas.width = gameContainer.clientWidth;
    canvas.height = gameContainer.clientHeight;
